...
void get_eth_index()
{
	...
	strncpy(ifreq_i.ifr_name,"h2-eth0",IFNAMSIZ-1);
...
void get_mac()
{
	...
	strncpy(ifreq_c.ifr_name,"h2-eth0",IFNAMSIZ-1);
...
void get_ip()
{
	...
	strncpy(ifreq_ip.ifr_name,"h2-eth0",IFNAMSIZ-1);
...
/****** OR
	...
	iph->daddr	= inet_addr("10.0.0.1"); // put destination IP address
...
/****** OR
	...
	iph->protocol	= 17;
	iph->saddr	= inet_addr(inet_ntoa((((struct sockaddr_in *)&(ifreq_ip.ifr_addr))->sin_addr)));
	iph->daddr	= inet_addr("10.0.0.3"); // put destination IP address
	total_len += sizeof(struct iphdr); 
	get_udp();
